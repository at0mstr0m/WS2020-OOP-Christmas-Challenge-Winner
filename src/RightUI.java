...
public class RightUI implements GameConfig, WaveContent, InputEventListener, TurretButtonListener {
    ...
    private final RightUIListener listener;
    private final Rectangle body;
    private final TurretButton[] turretButtons;
    private final Label moneyLabel;
    private final Label lifesLabel;
    ...
    public void handleMouseClick(int x, int y) {
        ...
        for (int i = 0; i < turretButtons.length; i++) {
            if (turretButtons[i].hitTest(x, y)) {
                if (turretButtons[i] == currentlyPlacingTurretButtonInstance || currentlyPlacingTurretButtonInstance == null) turretButtons[i].toggleBuildMode();
            }
        }
    }
